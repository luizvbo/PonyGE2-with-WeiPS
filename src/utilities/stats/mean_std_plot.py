...
def mean_std_plot(data_path: list, output_path=None):
    ...
    for method in data_path:
        ...
        mean = data.mean(0)
        sd = data.std(0)
        pl.plot(x, mean, 'k', color=colors[color_index])
        pl.fill_between(x, mean - sd, mean + sd,
                        alpha=0.5, edgecolor=colors[color_index],
                        facecolor=colors[color_index])
        color_index += 1
    pl.show()

mean_std_plot(['/tmp/data/ud/zdt1/ud_unpas_zdt1', '/tmp/data/ud/zdt1/ud_stratmeps_zdt1',
               '/tmp/data/ud/zdt1/ud_meps_zdt1', '/tmp/data/ud/zdt1/ud_rawps_zdt1',
               '/tmp/data/ud/zdt1/ud_nsga_2_zdt1'])